...
module Ivory.BSP.STM32G0B1.GPIO.Ports (
    gpioA
  , gpioB
  , gpioC
  , gpioD
...
gpioE = mkGPIOPort gpioe_periph_base rccenable rccdisable 4
  where
  rccenable  = modifyReg rcc_reg_iopenr $ setBit   rcc_iopenr_gpioeen
  rccdisable = modifyReg rcc_reg_iopenr $ clearBit rcc_iopenr_gpioeen
